# Direct MVI

![version](https://img.shields.io/badge/version-0.1.0-blue.svg)
![language](https://img.shields.io/badge/language-Kotlin-purple.svg)
![platform](https://img.shields.io/badge/platform-Android%20%7C%20Core-green.svg)
![license](https://img.shields.io/badge/license-Apache%202.0-orange.svg)

**Direct** — это MVI библиотека для Kotlin. Поддерживает kmp, а именно android, iosX64, iosArm64, iosSimulatorArm64, jvm и js таргеты.

С помощью библиотеки можно декларивно описывать ваши намерения и выбрать ту политику обработки, которая больше всего подходит.

---

## 📦 Как подключить

Библиотека доступна через JitPack.

**1. Добавьте репозиторий в `settings.gradle.kts`:**

```kotlin
dependencyResolutionManagement {
    repositories {
        google()
        mavenCentral()
        maven { url = uri("https://jitpack.io") } // <--- Вот это
    }
}
```

**2. Подключите нужные модули:**

```kotlin
dependencies {
    val directVersion = "0.2.1"

    // Основная логика
    implementation("com.github.driedfru1t.Direct:direct-core:$directVersion")
    
    // ViewModel автоматичеки тянет за собой и direct-core
    implementation("com.github.driedfru1t.Direct:direct-viewModel:$directVersion")
}
```

---

## 🚀 Быстрый старт

### 1. Описываем контракт
```kotlin
// Действия (Вход)
sealed interface HomeIntent : DirectIntent {
    data object Load : HomeIntent
    data class Click(val id: Int) : HomeIntent
}

// Состояние (Выход)
data class HomeState(
    val isLoading: Boolean = false, 
    val items: List<String> = emptyList()
) : DirectState

// Разовые события
sealed interface HomeEffect : DirectEffect {
    data object Navigate : HomeEffect
}
```

### 2. Пишем ViewModel
Нужно наследоваться от `DirectViewModel` и описать сценарии в блоке `intents`. Каждый сценарий можно гибко настроить и выбрать стратегию выполнения.

```kotlin
class HomeViewModel(
    private val repo: Repository
) : DirectViewModel<HomeIntent, HomeState, HomeEffect>() {

    override fun createInitialState() = HomeState()

    override fun handleIntents() = intents {
        
        // onLatest: эта стратегия гарантирует, что если придет новый интент пока старый ещё обрабатывается, 
        // то старый интент прекратит свою работу и начнет обрабатываться новый
        onLatest<HomeIntent.Load> {
            setState { copy(isLoading = true) }
            val items = repo.load()
            setState { copy(isLoading = false, items = items) }
        }

        // on: обычная обработка по очереди
        on<HomeIntent.Click> {
            setEffect { HomeEffect.Navigate }
        }
    }
}
```

### 3. Подписываемся на изминения стэйта
```kotlin
@Composable
fun AnotherScreen(){
    val viewModel = ... //получаете любыми способами viewModel
    val state by viewModel.state.collectAsStateWithLifecycle() // далее получаем стэйт
}
```

Вот пока все доступные функции для описания интентов без какой либо сложной настройки.
```kotlin
on<Intent>{  } 
onlatest<Intent>{  }
onSingle<Intent>{  }
onParallel<Intent>{  }
```
Ниже будет таблица с объяснением каждой стратегии

---

## ⚡ Гибкая настройка (DSL)

При помощи либы можно настроить и сложные сценарии, где нужны дополнительные условия или прочие трансформации потока с интентом. Так как интент предствляет собой поток, то в библиотеке предусмотрен специальный метод `transform{ }` где вы можете манипулировать интентом по своему желанию.
### 1. Пайплайны (`setup`)
Пример, где нужно отфильтровать лишние запросы и подождать ввода (debounce). Нужно учесть, что все настройки работают в том порядке в каком вы объявили, то есть сверху вниз(кроме блока catch и там где объявляется стратегия)

```kotlin
setup<Search> {
    // 1. Не шлем запросы, пока набрано меньше 3 символов
    filter { it.query.length > 2 } 
    
    // 2. Ждем 300мс, пока юзер перестанет печатать
    debounce(300)                  
    
    // 3. Если запрос не изменился — игнорируем
    distinct()                     
    
    // 4. Ловим ошибки прямо здесь
    catch { e -> ... }             

    // 5. И только потом запускаем загрузку (с отменой старой)
    latest { intent ->             
        // Логика поиска...
    }
}
```

### 2. Внешние данные (`listen`)
Store может слушать не только UI, но и базу данных, таймеры или сеть.

```kotlin
// Слушаем поток пользователя из БД
listen(repository.userFlow) { user ->
    // Как только в базе что-то поменялось — обновляем стейт
    setState { copy(currentUser = user) }
}

// Слушаем сеть(можно так же легко сделать сложную настройку)
listen(networkStatusFlow) {
    debounce(500) // Защита от "дребезга" соединения
    distinct()
    
    latest { isOnline ->
        if (isOnline) reloadData()
    }
}
```

---

## 🎛 Стратегии конкурентности

Главная фишка библиотеки. Вы сами решаете, что делать, если **новое** событие пришло, пока мы **всё ещё заняты** старым. Все политики доступны для `listen` и `on*`. Пока доступно только 4 политики обработки интентов.

| Стратегия                   | DSL                       | Как работает                                                          |
|:----------------------------|:--------------------------|:----------------------------------------------------------------------|
| **Serial**(Очередь)         | `on`/`serial`             | **Ждет** Новое событие встанет в очередь и выполнится после текущего. |
| **Latest**(Перезапуск)      | `onLatest`/`latest`       | **Отменяет** Старая работа бросается на полпути, запускается новая.   |
| **Dropping**(Анти-спам)     | `onSingle`/`dropping`     | **Игнорирует** Если мы заняты, новые клики просто не работают.        |
| **Concurrent**(Параллельно) | `onParallel`/`concurrent` | **Запускает** Всё работает одновременно, никто никого не ждет.        |

---

## 🔌 Middleware

Так же можно подключить некое подобие middleware. Эта штука позволяет вклиниться в жизненный цикл и не меняя ничего, как то отреагировать. Например логирование(Нужно наследоваться от интерфейса DirectMiddleware):
```kotlin
// Пример простого логгера
class LogMiddleware(val tag: String) : DirectMiddleware<Any, Any, Any> {
    override fun onIntent(intent: Any) = Log.d(tag, "➡️ Intent: $intent")
    override fun onStateChanged(old: Any, new: Any) = Log.d(tag, "🔄 State: $new")
}

// Подключаем одной строчкой
override fun handleIntents() = intents {
    install(LogMiddleware("HomeVM"))
}
```

## Inline feature store
Можно создать отдельный независимы стор и переиспользовать его. Для этого в блоке `intents`(для viewModel) или `runDsl`(если напрямую наследоваться от DirectStore) нужно испольщовать функцию `feature`. В ней вы указываете ваш стор и прописываете все контракты между вашим стором и подключаемым.

**1. Надо описать внедряемый стор**

Встраиваем ы стор ничего не знает о своём родителе и полностью самостоятелен
```kotlin
// Фича "Список новостей"
class NewsStore(scope: CoroutineScope) : DirectStore<NewsIntent, NewsState, NewsEffect>(scope) {
    override fun createInitialState() = NewsState()
    override fun handleIntents() = runDsl {
        onLatest<NewsIntent.Load> { /* логика загрузки */ }
    }
}
```
**2. Интрегрируем в родительский стор или ViewModel**

Родитель содердит ребёнка и и пробрасывает ему интенты
```kotlin
class HomeViewModel(private val newsStore: NewsStore) : DirectViewModel<HomeIntent, HomeState, HomeEffect>() {

    override fun createInitialState() = HomeState(
        news = newsStore.createInitialState() // Инициализируем вложенный стейт
    )

    override fun handleIntents() = intents {
        
        // Монтируем дочернюю фичу
        feature(
            store = newsStore,
            // 1. Синхронизация: когда у ребенка меняется стейт, обновляем свой
            state = { childState -> 
                setState { copy(news = childState) } 
            },
            // 2. Эффекты: слушаем ошибки или события навигации ребенка
            effects = { childEffect ->
                if (childEffect is NewsEffect.Error) setEffect { HomeEffect.ShowToast }
            },
            // 3. Маршрутизация: пересылаем нужные интенты ребенку
            intents = { parentIntent ->
                when (parentIntent) {
                    is HomeIntent.RefreshAll -> NewsIntent.Load
                    is HomeIntent.NewsAction -> parentIntent.innerIntent
                    else -> null
                }
            }
        )
    }
}
```

**Совет**

Для того чтобы можно было пробрасывать удобно интенты между родительским стором и дочерним можно создать интент обёртку
```kotlin
sealed interface ChildIntent : DirectIntent

sealed interface ParentIntent : DirectIntent {
    data class Wrapper(val intent: ChildIntent) : ParentIntent
}
```

---

## 📄 Лицензия

```
Apache License 2.0
Copyright 2026 Nikol
```